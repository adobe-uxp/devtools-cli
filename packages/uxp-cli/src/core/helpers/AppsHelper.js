...
class AppsHelper {
    ...
    static filterConnectedAppsForPlugin(connectedApps, pluginApplicableApps) {
        if (!pluginApplicableApps.length) {
            throw new Error("List of applicable apps for Plugin is empty.");
        }
        if (!connectedApps.length) {
            throw new Error("List of connected apps is Empty. Make sure the target application is running and try again.");
        }
