 ...
 *
 */
const { isPortAvailable } = require("../../utils/common");

function handleServiceStartCommand(argv) {
    // start the service at the given port.
    this.uxp.setMode('service');
    const prom = this.uxp.devToolsMgr.isDevToolsEnabled();
    const devToolsProm = prom.then((result) => {
        if (!result) {
            ...
            return this.uxp.devToolsMgr.enableDevTools();
        }
        return result;
    }).catch((err) => {
    ...
    return devToolsProm.then((result) => {
        ...
        const { port } = argv;
        return isPortAvailable(port).then((isAvailable) => {
            if (!isAvailable) {
            ...
            return this.uxp.serviceMgr.start(port);
        });
    });
}
