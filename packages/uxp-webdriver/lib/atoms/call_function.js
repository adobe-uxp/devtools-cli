...
function wdCallFunction() {
    ...
    function _recompileFunction(func) {
        if (typeof func === "string") {
            try {
                ...
                try {
                    return new Function("return (" + func + ").apply(null,arguments);");
                }
                catch(err) {
                    throw new Error("Passed function is not well-serializable! " + err);
                }
            }
        }
        return func;
    ...
    function callFunction(shadowHostIds, func, args, w3c, opt_unwrappedReturn) {
        ...
        var status = 0;
        try {
            var val = unwrap(args, cache);

            var returnValue = wrap(func.apply(null, val));
        }
        catch (error) {
